...
class OfflineManager:
    ...
    def cache_disposal_offline(self, user_id: str, latitude: float, longitude: float, images_data: List[str]) -> bool:
        ...
        try:
            ...
            for hotspot in hotspots:
                ...
                ''', (
                    ...
                    hotspot['longitude'],
                    hotspot['intensity'],
                    hotspot.get('expires_at', datetime.utcnow().isoformat())
                ))
            ...
            with app_context:
                ...
                for disposal in pending_disposals:
                    try:
                        ...
                        images_data = [img.encode() for img in disposal['images_data']]
                        ai_result = validate_disposal_with_ai_video(images_data[0] if images_data else b'')
                        
                        if ai_result['success']:
                            # Save to main database
                            new_disposal = Disposal(
                                user_id=disposal['user_id'],
                                timestamp=datetime.fromisoformat(disposal['timestamp']),
                            ...
                            daily_log = DailyDisposalLog(
                                user_id=disposal['user_id'],
                                date=datetime.fromisoformat(disposal['timestamp']).date(),
                                waste_sub_type=ai_result['waste_sub_type']
                            )
                        ...
                        logger.error(f"Error syncing disposal {disposal['id']}: {str(e)}")
                        sync_stats['failed_disposals'] += 1
                        continue
                
                logger.info(f"Sync completed: {sync_stats['synced_disposals']} synced, {sync_stats['failed_disposals']} failed")
